...
async def handle_input(message: types.Message):
    ...
    if message.chat.id == ADMIN_ID and admin_waiting.get(message.chat.id):
        parts = text.split('\n')
        if len(parts) < 2:
            await safe_send(message, 'Формат:api_keysecret_key')
            return
        try:
    ...
    if message.chat.id in user_pending:
        if text not in ('5', '10'):
            await safe_send(message, 'Выберите кнопкой: 5 или 10 секунд')
            return
        data = user_pending.pop(message.chat.id)
        if queue.full():
            await safe_send(message, '⛔ Очередь переполнена')
            return
        await queue.put((message, data['image_url'], data['prompt'], text, time.time()))
        await safe_send(message, f'⏳ Вы в очереди: {queue.qsize()}', ReplyKeyboardRemove())
        return

    lines = text.split('\n')
    if len(lines) < 2:
        await safe_send(message, '❌ Формат:ссылка_на_картинкупромпт')
        return

    image_url = lines[0].strip()
    prompt = ''.join(lines[1:]).strip()[:MAX_PROMPT_LEN]
    user_pending[message.chat.id] = {'image_url': image_url, 'prompt': prompt}
